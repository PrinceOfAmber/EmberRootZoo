    ...
    public void onUpdate(){
    	...
    	if (getDataManager().get(fadeTimer) > 0){
    		for (int i = 0; i < 5; i ++){
				...
				EmberRootZoo.proxy.spawnParticleMagicSmallSparkleFX(getEntityWorld(), location.xCoord, location.yCoord+1.35f, location.zCoord, 0, 0, 0, 107, 255, 28);
    		}
    	}
    	if (getDataManager().get(fadeTimer) == 0 && getDataManager().get(pacified)){
    		for (int i = 0; i < 20; i ++){
    			...
    			for (int j = 0; j < 2; j ++){
    				...
    				EmberRootZoo.proxy.spawnParticleMagicSparkleFX(getEntityWorld(), location.xCoord, location.yCoord+1.35f, location.zCoord, 0, 0, 0, 107, 255, 28);
    			}
    		}
